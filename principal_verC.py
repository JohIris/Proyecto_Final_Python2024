...
def ingresoUs():
    ...
    while intentos > 0:
        ...
        for dato in losDatosG:
            if aliasIng == dato['alias']:
                encontrado = True
                contraseña = input('Ingrese su contraseña: ')
                if contraseña == dato['clave']:
                    limpiar()
        ...
        if not encontrado:
            ...
            intentos -= 1
            print(f"Te quedan {intentos} intentos")
    print(Fore.LIGHTRED_EX + "Se han agotado los intentos. Intente nuevamente más tarde.")
...
def crearUs():
    existe = False
    nAlias = input("Ingresa un nombre de usuario: ").upper()
    for dato in losDatosG:
        if nAlias == dato['alias']:
            print(Fore.LIGHTRED_EX + "Nombre de usuario ya registrado!! Intente con otro...")
            existe = True
            break
    if not existe: 
...
# Para modificar usuario... cuando se ingresa una letra o simbolo el programa corre igual
# y modifica cualquier usario que encuentra con esa letra.